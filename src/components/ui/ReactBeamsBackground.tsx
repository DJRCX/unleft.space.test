...
function createStackedPlanesGeometry(
  ...
  let indexOffset = 0;
  let uvOffset = 0;
  const totalWidth = n * width + (n - 1) * 0;
...
const MergedPlanes = forwardRef<THREE.Mesh, MergedPlanesProps>(
  ({ material, width, count, height }, ref) => {
    ...
    const geometry = useMemo(
      () => createStackedPlanesGeometry(count, width, height, 100),
      [count, width, height]
    );
    useFrame((_, delta) => {
...
// ── Public wrapper ────────────────────────────────────────────────────────────
interface ReactBeamsBackgroundProps extends BeamsSceneProps {
  className?: string;
}

export function ReactBeamsBackground({
  className,
  ...sceneProps
}: ReactBeamsBackgroundProps) {
  return (

  ...
  return (
    <MotionConfig reducedMotion="user">
      <div className="relative z-10 mx-auto max-w-4xl text-center">
        ...
        >
          {/* Primary CTA — full glow */}
          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}>
            <HoverButton
              onClick={() => (window.location.href = "/contact")}
              className="inline-flex items-center shadow-[0_0_20px_rgba(124,58,237,0.4)]"
            >
              Start a Project <ChevronRight className="ml-2 h-4 w-4" />
            </HoverButton>
          </motion.div>

          {/* Secondary CTA — subtle glass style with HoverButton */}
          <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
            <HoverButton
              onClick={() => (window.location.href = "/projects")}
              className="inline-flex items-center gap-2 rounded-lg border border-[#2D2D44] bg-[#1A1A2E]/50 px-8 py-4 text-base font-medium text-[#E5E7EB] backdrop-blur-sm"
              style={
                {
                  "--circle-start": "rgba(192, 132, 252, 0.4)",
                  "--circle-end": "rgba(124, 58, 237, 0.3)",
